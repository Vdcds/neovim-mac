...
# macOS support
    ...
    if ! command -v brew >/dev/null 2>&1; then
        echo -e "${RED}Homebrew not found. Please install Homebrew first: https://brew.sh${RC}"
        exit 1
    fi
    brew install ripgrep fd fzf neovim luarocks go shellcheck webp node npm make
    # python3 is pre-installed on macOS, but ensure virtualenv is available
